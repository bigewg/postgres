...
INSERT INTO t_dbls (col1, col2)
SELECT n, n+1 FROM generate_series(1, 500000) n;
...
INSERT INTO t_dbls (col1, col2)
SELECT 1, 2 FROM generate_series(1, 100000);

INSERT INTO t_dbls (col1, col2)
SELECT 2, 3 FROM generate_series(1, 100000) n;

INSERT INTO t_dbls (col1, col2)
SELECT 3, 4 FROM generate_series(1, 100000) n;

INSERT INTO t_dbls (col1, col2)
SELECT 4, 5 FROM generate_series(1, 100000) n;

INSERT INTO t_dbls (col1, col2)
SELECT 5, 6 FROM generate_series(1, 100000) n;
...
---------------------------------------------------------------------------------------
-- 2. вариант с row_number - должен работать в MS SQL Server?
---------------------------------------------------------------------------------------
TRUNCATE TABLE t_dbls;
...
SELECT ctid, * FROM t_dbls;

DELETE FROM t_dbls T
USING	(
		SELECT ctid, row_number() OVER (PARTITION BY col1, col2 ORDER BY ctid) rn
		FROM t_dbls
		) R
WHERE T.ctid = R.ctid
  AND R.rn > 1;
...
SELECT * FROM t_dbls ORDER BY col1 LIMIT 100;
...
CREATE TEMP TABLE tt_ctid (id tid);
...
-- Execution Time: 395.629 ms (Ќо затраты на построение врем€нки)