...
SELECT book_name, isbn FROM book_store.book;
...
SELECT ns::text FROM generate_series(1, 75000) ns;

INSERT INTO t_two (one_id, str)
SELECT id, str || '+1' FROM t_one
UNION 
SELECT id, str || '+2' FROM t_one
;

INSERT INTO t_one (str)
SELECT ns::text FROM generate_series(1, 200000) ns;
...
SELECT	G.genre_name,
		B.book_name
FROM book_1 B
INNER JOIN genre G USING (genre_id);-- ON G.genre_id = B.genre_id;
...
SELECT 	B.book_name, author_name
		--
		, G.genre_name 
FROM book B
...
INNER JOIN	(
			...
			FROM book_author BA
			INNER JOIN author A ON A.author_id = BA.author_id
			WHERE A.author_name LIKE 'А%'
			) R ON R.book_id = B.book_id;
--------------------------------------------------------------------------------------------

		UPDATE book_1 SET genre_id = 18 WHERE book_id = 9;
		UPDATE book_2 SET genre_id = 18 WHERE book_id = 9;
...
SELECT * FROM book_1
UNION -- ALL
SELECT book_name, published FROM book_2	
...
ROLLBACK; 
START TRANSACTION;
DELETE FROM book_2 WHERE book_id > 100;

DELETE FROM book_2 B2
USING genre
WHERE B2.genre_id = genre.genre_id AND genre.genre_name = 'Биографии авиакострукторов'
